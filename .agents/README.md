...
| --------------- | ---------------------------------------------------------------- | -------------------- |
| `<domain>`      | the integration domain, i.e. the `DOMAIN` constant in `const.py` | `hagelschutz_vkf`    |
| `{ClassPrefix}` | the class name prefix used by every integration class            | `HagelschutzVkf`     |
